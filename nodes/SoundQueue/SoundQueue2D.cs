...
{
	[Tool]
	public partial class SoundQueue2D : Node2D
	{
		// EXPORTS //////////////////////////////////////////
        /// <summary>
		...
		private Array<AudioStreamPlayer2D> m_players;
		...
		public override void _Ready()
		{
			...
			if (GetChild(0) is AudioStreamPlayer2D child)
			{
				m_players.Add(child);
				
				for(int i = 0; i < m_Copies; i++)
				{
					AudioStreamPlayer2D duplicate = child.Duplicate() as AudioStreamPlayer2D;
					AddChild(duplicate);
		...
		public override string[] _GetConfigurationWarnings()
		{
			...
			if ((GetChild(0) is AudioStreamPlayer2D) == false)
			{
				return new string[] { "First child is not an AudioStreamPlayer2D!" };
			}
